...
def mouse_icon(c,x,top,button):
    """Small top-view mouse: user's left is always on the drawing's left."""
    mw=29;mh=42;bottom=top-mh
    box(c,x,bottom,mw,mh,WHITE,BLUE,10)
    c.saveState()
    clip=c.beginPath();clip.roundRect(x,bottom,mw,mh,10);c.clipPath(clip,stroke=0)
    c.setFillColor(BLUE)
    c.rect(x if button=='left' else x+mw/2,bottom+24,mw/2,18,fill=1,stroke=0)
    c.restoreState()
    c.setStrokeColor(BLUE);c.setLineWidth(.9)
    c.line(x+mw/2,bottom+24,x+mw/2,top)
    c.line(x,bottom+24,x+mw,bottom+24)
    box(c,x+12,top-16,5,10,WHITE,BLUE,2)
    txt(c,'左鍵' if button=='left' else '右鍵',x-5,bottom-3,42,10,14,BLUE,True)
...
def arrow(c,a,b,col=AMBER,width=1.5,heads=1,tipsize=6):
    c.setStrokeColor(col);c.setFillColor(col);c.setLineWidth(width);c.line(*a,*b)
    def tip(p,q):
        angle=math.atan2(p[1]-q[1],p[0]-q[0]);r=tipsize
...
def panel_dim(c,a,b,label,vertical=False):
    arrow(c,a,b,BLUE,.55,2,3)
    cx=(a[0]+b[0])/2;cy=(a[1]+b[1])/2
    c.saveState();c.translate(cx-3 if vertical else cx,cy if vertical else cy+3)
    if vertical:c.rotate(90)
    size=8.5;tw=pdfmetrics.stringWidth(label,'JHB',size)
    c.setFillColor(WHITE);c.rect(-tw/2-1,-1,tw+2,10,fill=1,stroke=0)
    c.setFillColor(BLUE);c.setFont('JHB',size);c.drawCentredString(0,0,label);c.restoreState()

def sheet(c,m,num,x,top,w,h,dimensions=True):
    ...
    for idx,p in enumerate(m['panels'],1):
        ...
        # Keep each panel's aspect ratio and relative placement, while opening
        # space for dimensions. This drawing is a reading aid, not the cut file.
        ds=sc*.70 if dimensions else sc
        if dimensions:px+=12;py-=14
        pw=sz['width']*ds;ph=sz['height']*ds
        c.setStrokeColor(RED);c.setLineWidth(.8);c.rect(px,py-ph,pw,ph,fill=0,stroke=1)
        for e in p['engraving']:
            c.setStrokeColor(BLUE);c.setLineWidth(.4);c.rect(px+e['x']*ds,py-(e['y']+e['height'])*ds,e['width']*ds,e['height']*ds,fill=0,stroke=1)
        if dimensions:
            panel_dim(c,(px,py+7),(px+pw,py+7),f"{idx:02d} | {sz['width']:g}")
            panel_dim(c,(px-6,py-ph),(px-6,py),f"{sz['height']:g}",True)
        else:
...
def lesson(c,title,sub,draw,items,tag='基本操作',mouse_buttons=None):
    ...
    for i,(h,b) in enumerate(items):
        if mouse_buttons:
            # Room beside icon for title and beneath it for the full paragraph.
            x=M+448;ww=CW-448;button=mouse_buttons[i]
            st=ParagraphStyle('mb',fontName='JH',fontSize=14.5,leading=20.3,wordWrap='CJK')
            p=Paragraph(escape(b),st);_,bh=p.wrap(ww-26,900);ht=bh+79
            box(c,x,y-ht,ww,ht)
            mouse_icon(c,x+15,y-9,button)
            txt(c,h,x+65,y-17,ww-80,15,21,BLUE,True)
            txt(c,b,x+13,y-69,ww-26,14.5,20.3)
            y-=ht+10
        else:y=card(c,h,b,M+448,y,CW-448,14.5)
    end(c)
...
def doc(c):
    ...
    yy=t
    for h,b in [('頂板、底板各 1 件','80 × 60 mm；上下各蓋住四面牆。'),('前牆、後牆各 1 件','80 × 46 mm。'),('左牆、右牆各 1 件','56 × 46 mm。')]:yy=card(c,h,b,M+455,yy,CW-455)
    ...
    lesson(c,'01｜開新檔，先用毫米','頁面是排版範圍；不是建築物大小，也不是機器工作台。',newdraw,[('按 Ctrl + N','或選「檔案 → 新增」。'),('設定單位及頁面','繪圖單位選 mm；頁面寬 300、高 200。按確定。'),('完成後應看到','空白橫向頁面。接著開始畫第一塊牆板。')])
    ...
    lesson(c,'03｜不要填色，只保留外框','課堂識別：紅色外框 = 切割；藍色線條 = 表面雕刻。',colordraw,[('先選牆板','用左邊黑箭頭選取工具，左鍵點矩形。'),('清除填色','在右邊色盤左鍵點「無色」× 色塊。這是滑鼠操作，不是按鍵盤 X。'),('設定外框','右鍵點紅色色塊。若整塊變紅，按 Ctrl+Z，再用右鍵。切割線寬由老師指定。')],mouse_buttons=['left','left','right'])
    ...
    def layoutdraw(c,x,y,w,h):
        ...
        txt(c,'W1 排版｜上方：編號 | 寬；左邊：高',x,y-303,w,14,21,BLUE,True)
        txt(c,'標示單位 mm；SVG 頁面 300 × 200 mm。\n尺寸只供本頁閱讀，切割檔沒有這些標示。',x,y-337,w,14,20)
    lesson(c,'09｜排版及儲存：交 CDR 給老師','一套完整模型 = 數量 1；不是填入木板零件數目。',layoutdraw,[('外框留空隙','所有板放在頁面內；範本外框相隔至少 6 mm。移動時把該板和門窗一起選取／群組。'),('按 Ctrl + S 儲存','檔案類型用 CorelDRAW（CDR）。\n例：2B_GROUP 2_W1_1.cdr'),('兩張範本怎樣交','每張 SVG 各開一個新文件，匯入後分別儲存，檔尾加 _S01、_S02。\n例：2B_GROUP 2_W3_1_S01.cdr\n兩個檔案都附全圖截圖。')])
    ...
    # The student edition ends after W6; the former pages 32-33 are omitted.
    return
...
def model_pages(c,m,i):
    ...
    txt(c,'上方：編號 | 寬；左邊：高。單位 mm。圖示間距已調整，切割 SVG 沒有這些標示。',M,t-286,CW,12,17,MUTED)
    for j,p in enumerate(m['panels']):
        col=j//6;row=j%6;x=M+col*(CW+12)/3;y=t-317-row*22
        sz=p['cut_size_mm']
        txt(c,f"{j+1:02d} {p['module_id']} {p['name_zh']}  {sz['width']:g} × {sz['height']:g} mm",x,y,(CW-24)/3,12.5,18,INK)
    end(c)
...
def build():
    ...
    print('Student pages',len(PAGES),'PDF',PDF)

def package():
    pack=ROOT/'output/coreldraw-wood-building-complete-pack.zip'
    with zipfile.ZipFile(pack,'w',zipfile.ZIP_DEFLATED) as z:
        z.write(PDF,'01_學生指南.pdf')
        english=PDF.with_name('coreldraw-2019-wood-building-student-guide-en.pdf')
        if english.exists():z.write(english,'01_Student_Guide_English.pdf')
        z.write(TEACHER,'02_教師設定表.pdf')
        z.write(OUT/'先讀我_README.md','先讀我_README.md')
        for f in sorted((OUT/'templates').rglob('*')):
            if f.is_file():z.write(f,f.relative_to(OUT).as_posix())
    with zipfile.ZipFile(pack) as z:
        assert z.testzip() is None
        assert z.read('01_學生指南.pdf')==PDF.read_bytes()
        assert len([p for p in z.namelist() if p.endswith('.svg')])==9
    print('Updated complete pack',pack)
