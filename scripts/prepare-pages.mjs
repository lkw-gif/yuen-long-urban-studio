...
for (const route of ['concepts', 'bridge-workshop', '3d-design', 'district-design']) {
  mkdirSync(resolve(root, route), { recursive: true });
  copyFileSync(resolve(root, `${route}.html`), resolve(root, route, 'index.html'));
}
for (const path of ['index.html', '_next', 'concepts/index.html', 'bridge-workshop/index.html', '3d-design/index.html', 'district-design/index.html']) {
  if (!existsSync(resolve(root, path))) throw new Error(`Missing Pages output: ${path}`);
}
writeFileSync(resolve(root, '.nojekyll'), '');
console.log('Pages ready: home, concepts, bridge-workshop, 3d-design and district-design.');
