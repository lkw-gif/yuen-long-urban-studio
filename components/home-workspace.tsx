...
import { Box, ArrowUpRight, Layers3, ChevronRight, MapPin, CircleHelp, MousePointer2, TreePine, Route, Scan, Sun, Focus, Plus, Minus, RotateCcw, Move, Mouse, Maximize2, Grid2X2, Eye, EyeOff, ChevronDown, Navigation2, Settings2, Circle, Triangle, Download } from 'lucide-react';
...
export default function HomeWorkspace() {
 ...
 useEffect(()=>{if(!inspectorOpen)return;const close=(e:KeyboardEvent)=>{if(e.key==='Escape')setInspectorOpen(false);};window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close);},[inspectorOpen]);
 ...
 const onReady=useCallback((ready=true)=>setReady(ready),[]);
 ...
 return <main className={'studio '+(inspectorOpen?'show-inspector':'')}>
  ...
  <div className="workspace">
   ...
   <aside id="scene-inspector" className="right-panel" aria-label="物件與場景設定">
    <div className="mobile-layer-settings"><h3>場景圖層</h3>{(['buildings','parks','roads','curbs'] as LayerName[]).map((layer,i)=><label key={layer}>{['建築量體','公園與綠化','道路與地面','馬路邊線'][i]}<Switch checked={visibility[layer]} onCheckedChange={v=>toggle(layer,v)} aria-label={['手機顯示建築','手機顯示綠化','手機顯示道路','手機顯示路緣'][i]}/></label>)}<label>選取建築<select value={selected} onChange={e=>setSelected(e.target.value)} aria-label="選取街區建築">{BUILDINGS.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label></div><div className="inspector-heading"><Settings2 size={15}/>屬性面板<small>INSPECTOR</small></div>
